...
const passResetToken = (payload, pass) =>
  jwt.sign(payload, pass, { expiresIn: 3600 });
  ...
  newPassRcovery.save(async (err, doc) => {
    if (err) res.json({ status: "failed", message: err });
      ...
      let resetLinkSent = await emailCheck.confirmation({
        ...
        text: "",
        html: `<b>To Change your passowrd please <a href="http://localhost:4200/resetpassword/${payload.id}/${resetToken}">Click here!</a></b>`,
      });
      if (resetLinkSent)
        res.json({
          status: "success",
          message:
            "Email containg reset link successfuly sent. Please check your email.",
        });
      else
        res.json({
          status: "failed",
...
exports.resetPassword = async (req, res) => {
  ...
  if (pass !== confirmPass)
    return res.json({
      status: "failed",
      message: "Passwords mismatch please check your inputs",
    });
